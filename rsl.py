...
from lib.inversion import inversion_ols_any_degree as els

# cli
parser = argparse.ArgumentParser()
parser.add_argument("yml", help="configuration file")
parser.add_argument(
    "--debug", default=False, action="store_true", help="increase verbosity"
)
...
ng = nx * ny
trend, trendci = np.full((nx, ny), np.nan, dtype=float), np.full(
    (nx, ny), np.nan, dtype=float
)
accel, accelci = np.full((nx, ny), np.nan, dtype=float), np.full(
    (nx, ny), np.nan, dtype=float
)
...
dvars_2d["accel"] = {
    ...
    "units": "%s/%s/%s"
    % (conf["output_z_unit"], conf["output_t_unit"], conf["output_t_unit"]),
...
writer(conf["output"], x, y, t, dvars_2d, dvars_4d)
